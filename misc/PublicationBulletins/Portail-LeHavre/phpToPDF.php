...
//			$l=4*$i+4;
			...
			// ajouter la l&eacute;gende si elle doit être
			if ($droites[$i][3] != "")
			{
	...
	// Par d&eacute;fault, le texte sera normal et à gauche...
	// Fonction destin&eacute;e à dessiner un tableau dans un file.pdf
	function drawTableau(&$pdf, $tableType, $headerType, $headerDatas, $datasType, $datas)
	{
		...
		for($i=0; $i<$nbCol; $i++) 
		{
			...
			// par d&eacute;faut, le texte est centr&eacute; à gauche, non italic, non soulign&eacute; et non gras.
		...
		// Si l'utilisateur veut un alignement sp&eacute;cifique pour la première colonne. Sinon, T_ALIGN  prend le dessus...
		if (isset($headerType['T_ALIGN_COL0']))
			$header_type[0]['T_ALIGN'] = $headerType['T_ALIGN_COL0'];

		// Si l'utilisateur veut un fond color&eacute; sp&eacute;cifique  pour la première colonne. Sinon, BG_COLOR  prend le dessus...
		...
		for ($i=0 ; $i<count($datas) ; $i+=$nbCol)
		{
			...
			//*********************************************************************
			// si l'utilisateur a pr&eacute;cis&eacute; un alignement pour la première colonne, on l'applique ici
			if (isset($datasType['T_ALIGN_COL0']))
				$data[0]['T_ALIGN'] = $datasType['T_ALIGN_COL0'];
				
			// Si l'utilisateur a pr&eacute;cis&eacute; une couleur de fond pour la première colonne, on l'applique ici.
			...
			for ($j=$i ; $j<$i+$nbCol ; $j++)
			{
				...
				// de la cellule fusionn&eacute;e dans la première cellule et "COLSPAN2" dans la deuxième cellule.
