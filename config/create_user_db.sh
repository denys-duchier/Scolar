...
echo su -c "psql -U $POSTGRES_USER  $db_name < $SCODOC_DIR/misc/create_user_table.sql" $POSTGRES_USER
su -c "psql -U $POSTGRES_USER  $db_name < $SCODOC_DIR/misc/create_user_table.sql" $POSTGRES_USER
